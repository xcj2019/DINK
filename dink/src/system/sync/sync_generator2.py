...
if __name__ == "__main__":
    ...
    text = '/* ----header---- */\n'
    text += '/* common header */\n'
    text += '#include "dink/dink.h"\n'
    text += '#include <dink/callback_queue.h>\n'
    ...
    text += '{\n'
    text += '    dink::NodeHandle nh_rcv;\n'
    text += '    dink::CallbackQueue rcv_callbackqueue;\n'
    text += '    nh_rcv.setCallbackQueue(&rcv_callbackqueue);\n'
    text += '    dink::Subscriber %s_sub = nh_rcv.subscribe("%s", 5, %s_callback);\n' % (data['sync_sub'].split('/')[-1], data['sync_sub'].split('/')[-1], data['sync_sub'].split('/')[-1])
    text += '    while (nh_rcv.ok()) {\n'
    text += '        rcv_callbackqueue.callAvailable(dink::WallDuration(1.0f));\n'
    text += '        pthread_mutex_lock(&mutex);\n'
